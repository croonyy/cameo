        ...
        <n-form-item :label="schema.label" :path="schema.field">
          ...
          <template v-else-if="schema.component === 'NCheckbox'">
            <n-checkbox-group v-model:value="formModel[schema.field]">
              <n-space>
                <n-checkbox
                  v-for="item in getComponentOptions(schema)"
                  :key="item.value"
                  :value="item.value"
                  :label="item.label"
                />
      ...
      >
        ...
        >
          <n-button
            ...
            :loading="loadingSub"
            attr-type="submit"
            >{{ getProps.submitButtonText || t('button.search') }}</n-button
          >
          <n-button
            v-if="getProps.showResetButton"
            v-bind="getResetBtnOptions"
            @click="resetFields"
            >{{ getProps.resetButtonText || t('button.reset') }}</n-button
          >
          <n-button
            type="primary"
            text
          ...
          >
            ...
            {{ overflow ? t('button.expand') : t('button.collapse') }}
          </n-button>
        </n-space>
      </n-gi>
    </n-grid>
...
<script lang="ts">
  ...
  export default defineComponent({
    ...
    setup(props, { emit, attrs }) {
      ...
      function getRawComponentProps(schema: FormSchema) {
        return typeof schema.componentProps === 'function'
          ? schema.componentProps({ formModel, schema })
          : schema.componentProps ?? {};
      }

      function getComponentOptions(schema: FormSchema) {
        const componentProps = getRawComponentProps(schema) as { options?: any[] };
        return componentProps.options ?? [];
      }

      function getComponentProps(schema: FormSchema) {
        const compProps = getRawComponentProps(schema);
